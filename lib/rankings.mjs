...
/** @typedef {{id: string, title: string, image: string, tier: TierId | null, reason: string}} RankingItem */
...
const uploadImagePattern = /^\/uploads\/[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}\.(png|jpg|gif|webp)$/;
...
export function validateRanking(value, { published = false } = {}) {
  ...
  const items = Array.from(value.items, item => {
    if (!record(item, ["id", "title", "image", "tier", "reason"]) ||
    ...
    ids.add(item.id);
    return { id: item.id, title: item.title, image: item.image, tier: item.tier, reason: item.reason };
  });
