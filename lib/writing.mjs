...
import { publishProgress } from "./publish-progress.mjs";
/** @param {{ stage: string, label: string, url?: string }} value */
const ignoreProgress = value => { void value; };
...
export function serializeEntry(entry, draft) {
  ...
  if (entry.trashed !== undefined && typeof entry.trashed !== "boolean") throw new Error("Trash status must be boolean.");
  if (entry.trashed && !draft) throw new Error("Restore this article from Trash before publishing.");
  ...
  const completeMetadata = { ...metadata, ...presentation(entry),
    ...(entry.trashed ? { trashed: true } : {}),
...
export function readEntry(source, path) {
  ...
  if (meta.trashed !== undefined && typeof meta.trashed !== "boolean") throw new Error("Trash status must be boolean.");
  ...
  return { section, slug, language, title: String(meta.title || ""), date: String(meta.date || ""), description: String(meta.description || ""), body: match[2], draft: meta.draft !== false || meta.trashed === true,
    ...presentation(meta),
    ...(meta.trashed ? { trashed: true } : {}),
...
function repositoryWriter(api, disconnect, wait) {
  ...
  return {
    ...
    async images() {
      let files;
      try { files = await api(endpoint("site-public/uploads") + "?ref=main"); }
      catch (error) { if (error.status === 404) return { images: [], truncated: false }; throw error; }
      if (!Array.isArray(files)) throw new Error("Could not list uploaded images.");
      return { images: files.filter(file => file.type === "file" && validUploadPath(file.path)).map(file => ({ image: file.path.slice("site-public".length), title: file.name })), truncated: files.length >= 1000 };
    },
    ...
    async publish(path, source, sha, onProgress = ignoreProgress) {
      onProgress({ stage: "checking", label: "Checking saved versions…" });
      ...
      if (original && readEntry(decodedText(original.content), path).trashed) throw new Error("Restore this article from Trash before publishing.");
      ...
      if (translation && readEntry(decodedText(translation.content), target).trashed) throw new Error("Restore the other language version from Trash before publishing.");
      const requestId = crypto.randomUUID();
      ...
      onProgress({ stage: "queued", label: "Queued on GitHub. Waiting for a runner…" });
      for (let attempt = 0; attempt < 30; attempt++) {
        await wait(10_000);
        const result = await api(workflow + "/runs?event=workflow_dispatch&branch=main&per_page=100");
        ...
        if (run) {
          ...
          if (Number.isSafeInteger(run.id) && run.id > 0) {
            try { jobs = (await api(repo + `/actions/runs/${run.id}/jobs?per_page=100`)).jobs || []; }
            catch { /* Status detail failure must not redispatch a paid publication. */ }
          }
          onProgress(publishProgress(run, jobs));
        }
        if (run?.status !== "completed") continue;
...
export function localWriter(fetcher = fetch) {
  ...
  return {
    ...
    publish: async (path, source, sha, onProgress = ignoreProgress) => {
      onProgress({ stage: "translating", label: "Translating and saving both versions locally…" });
      const result = await api("/api/publish", { method: "POST", body: JSON.stringify({ path, source, sha }) });
      onProgress({ stage: "local", label: "Saved. Both versions are visible in the local preview only." });
      return result;
    },
